...
micSim <- function(initPop, immigrPop=NULL, transitionMatrix, absStates=NULL, initStates=c(), initStatesProb=c(), 
  ...
  # cat("cS: ",cS, "-> dS: ",dS,"\n" )
   ...
   if("m" %in% cS)
    return(FALSE)
   if((!T %in% (cS %in% fert)) | (!T %in% (dS %in% fert))) 
    return(FALSE)
   cSFert <- cS[which(cS %in% fert)] 
   dSFert <- dS[which(dS %in% fert)] 
   tr <- paste(cSFert,dSFert,sep="->")
   if(paste(cS[-which(cS==cSFert)],collapse="/")!=paste(dS[-which(dS==dSFert)],collapse="/"))
     return(FALSE)
   if(tr %in% fertTr){
    return(TRUE)
   } else {
